...
function contact(event) {   //event is what gets passed in by onSubmit
  ...
  emailjs //this is the js that sends my account info and the form info to emailjs
    ...
    .then(() => { //so AFTER all the info gets sent successfully, loading state is unloaded and success state loads
      loading.classList.remove("modal__overlay--visible"); //unload loading
      success.classList += " modal__overlay--visible"; //show success
    })
    .catch(() => { //BUT, if anything goes wrong anywhere, this unloads loading state and tells user it didn't work.
...
function toggleModal() { //call fn by clicking some button
    if (isModalOpen) { //if isModalOpen is true (modal=open)
